module dynamics
  ...
  subroutine setup_dynamics()
    ...
    allocate( dV_dq(nbasis_ev,nbasis_ev,nclass) )
    allocate( ci(nbasis_ev) )
    allocate( xe(nbasis_ev), pe(nbasis_ev) )
    allocate( Nnk(nbasis_ev) )
  ...
  subroutine init_condition()
    ...
    real*8 :: rnd1, rnd2 !,rnd_xp(2*nbasis_e)
    real*8, dimension(nbasis_ev):: rnd_nk, rnd_qk 
    real*8 nk(nbasis_ev), qk(nbasis_ev)
    integer i    
    ...
    H       = ham_evDiab(R)
    dV_dq   = delHeDiab_delR(R)

    Nnk   = 0
    Nnk(1)= 1 !state_vec(istate)   ! electronic state 1, v=0
    
    call random_number(rnd_nk)
    nk = Nnk + ngamma*(2*rnd_nk-1)  !ngamma*[2*rnd_xp(1)-1, 2*rnd_xp(2)-1]
    call random_number(rnd_qk)
    qk = 2*pi*rnd_qk  ![2*pi*rnd_xp(3), 2*pi*rnd_xp(4)]
    ...
    write(*,*) "eng",   calc_tot_eng_Hn(Rdot, R, pe, xe)
    write(*,*) "nk: ",nk!, qk
  ...
  subroutine compute_force(xe, pe, dV_dq, force)
    ...
    real*8,intent(in) :: dV_dq(nbasis_ev,nbasis_ev,nclass_c)
    real*8,intent(out):: force(nclass_c)
    integer k1,k2
    ...
    do i = 1, nclass_c
      ...
      do k1=1,nbasis_ev
        do k2=1,nbasis_ev
          if (k1==k2) force(i) =force(i)+ (0.5*pe(k1)**2+0.5*xe(k1)**2 -ngamma)*dV_dq(k1,k1,i)
          if (k1<k2)  force(i) =force(i)+ (pe(k1)*pe(k2) + xe(k1)*xe(k2))*dV_dq(k1,k2,i)
        enddo
      enddo
      force(i) = -force(i)
    enddo
  ...
  subroutine compute_force_ci(xe, pe, dV_dq, forc)
    implicit none
    integer i
    real*8,intent(in), dimension(nbasis_ev)  :: xe, pe
    real*8,intent(in) :: dV_dq(nbasis_ev,nbasis_ev,nclass_c)
    real*8,intent(out):: forc(nclass_c)
    complex*16 ci(nbasis_ev)

    ci = ck_xp(xe,pe)
    do i = 1, nclass
      !force(i) =- real(sum(conjg(ci)*matmul(delV_diab(:,:,i), ci)))
      forc(i) = - expec_val(ci, dV_dq(:,:,i), ci)
    enddo
  end subroutine compute_force_ci

  subroutine evolve_classical(q, qdot, force) !! dummy variables for (R, Rdot, foRce; accR)
    ! Velocity Verlet
    implicit none
    real*8, intent(inout) :: q(:), qdot(:), force(:)  ! nclass_c
  ...
  function xedot(H_e, pe) result(xe_dot)
    implicit none
    real*8, intent(in)  :: H_e(nbasis_ev, nbasis_ev)
    real*8, intent(in), dimension(nbasis_ev)  :: pe
    real*8  xe_dot(nbasis_ev)
  ...
  function pedot(H_e, xe) result(pe_dot)
    implicit none
    real*8, intent(in)  :: H_e(nbasis_ev, nbasis_ev)
    real*8, intent(in), dimension(nbasis_ev)  :: xe
    real*8  pe_dot(nbasis_ev)
  ...
  subroutine evolve_quantum(xe, pe, Hel_k1, Hel_k4)
    ...
    real*8, intent(inout), dimension(nbasis_ev)  :: xe, pe
    real*8, intent(in), dimension(nbasis_ev, nbasis_ev) :: Hel_k1, Hel_k4  
    real*8, dimension(nbasis_ev) :: k1x, k2x, k3x, k4x
    real*8, dimension(nbasis_ev) :: k1p, k2p, k3p, k4p
    real*8  Hel_k23(nbasis_ev, nbasis_ev)
      ...
      traj_evolve: do ns = 1, nsteps
        ...
        if ( (mod(ns, nwrt_traj)==1) )  then !(mod(ns, nwrt_traj)==1 )
          if (ntraj==1) then 
            ...
            if (istr_eng==1) tot_energy(nt,j) = calc_tot_eng_Hn(Rdot,R,pe,xe) 
  ...
  function calc_pop_ci(ci, istate) result(popn)
    implicit none
    complex*16, intent(in) :: ci(nbasis_ev)
  ...
  function calc_state(xe, pe) result(k_state)
    ...
    real*8 var(nbasis_ev), var2(nbasis_e)
    ! write(*,*) "state (xe,pe): ", xe, pe
    ! ci_sq = (xe)**2 + (pe)**2
    var = 0.5*((xe)**2 + (pe)**2) - ngamma
    var2(1) = sum( var(1:nbasis_v) ) ! - ngamma
    var2(2) = sum( var(nbasis_v+1:2*nbasis_v) ) !- ngamma

    !isStatek(k) = all(abs(var-state_vec(k)) <= ngamma, 1)
    isState1 = all(abs(var2-N_s1) <= ngamma, 1)
    isState2 = all(abs(var2-N_s2) <= ngamma, 1)
  ...
  function calc_sum_nk(xe,pe) result(sum_nk)
    ...
    real*8, intent(in), dimension(nbasis_ev) :: xe, pe
    real*8 sum_nk
    sum_nk   = sum(0.5*(xe)**2 + 0.5*(pe)**2) - nbasis_ev*ngamma
  end function calc_sum_nk

  function calc_tot_eng_ci(ci, H) result(tot_eng)
    implicit none
    complex*16, intent(in)  :: ci(nbasis_ev)
    real*8, intent(in)  :: H(nbasis_ev,nbasis_ev) ! H_e = ham_eDiab(Rn)
